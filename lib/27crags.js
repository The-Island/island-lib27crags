...
var createTickObj = function($, els) {
  ...
  if (obj.type === 'b') {
    var grade = _.find(boulderMap, function(item) {
      return item.hueco === obj.grade;
    });
    if (grade) {
      obj.grade = grade.font;
    }
  }
  else if (obj.type === 'r') {
    var grade = _.find(routeMap, function(item) {
      return item.yds === obj.grade;
    })
    if (grade) {
      obj.grade = grade.french;
    }
  }
